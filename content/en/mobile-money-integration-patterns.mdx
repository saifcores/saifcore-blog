...
excerpt: >-
  Wave, Orange Money, Free Money: callback traps, HMAC verification, and
  reconciliation jobs — architected so SUCCESS never pretends to be settled.
kind: code
...
Mobile money rails in West Africa are not uniform. Providers expose different callback semantics, settlement windows, and retry behaviours.

A payment marked `SUCCESS` by the provider is not always a settled transaction. Building as though it is will surface as a reconciliation nightmare six months into production. The sharper vocabulary lives in [the callback that said SUCCESS](/articles/when-success-is-not-settled) — this article is the integration pattern that prevents the trap.
