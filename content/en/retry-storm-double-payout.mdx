...
That is the version that shipped. The polite title says **twice**. The logs said **three**.

[The callback that said `SUCCESS`](/articles/when-success-is-not-settled) lied with a word. This one lied with silence.

## Thursday, 16:47

Payout `PAY-33108`: **85,000 XOF**, one merchant, one order. The mobile money provider does not go down. It goes **slow** — four seconds, then five, then eight. Usual time is a few hundred milliseconds. The client's timeout is three seconds.

At 16:47:03 the client decides the first request failed.

It had not failed. It was still in flight.
...
Within ninety seconds the same payout command had been submitted **three times**. On the provider side, all three would have succeeded — three separate transfers, same amount, same recipient, one sale.

Finance would not have seen it that afternoon. They would have seen it in an end-of-day file — or days later — with no trail back to a three-second timeout.
...
The interview question is not whether you know the word. It is: **what is the key made of, and can a retry change it?**
...
All three retries carried the same key. The server saw calls two and three as duplicates of a command already in flight, returned the original response, and never issued a second transfer.
...
| **Support**     | A merchant asking why they were paid three times    | No ticket                              |
...
If the key had been a fresh UUID on each retry, Thursday's payout would have landed three times. Friday's settlement file would have been _too right_ — extra credits, not missing rows. Product would have called it a win. Finance would have called it a leak. Someone would have been on the phone with the merchant, explaining a refund for money they had already spent.
...
Same rule as in [scalable fintech systems](/articles/scalable-fintech-systems): ten retries must equal **one** economic intent. If the key can change between attempts, the safety net was never there.
...
A word lied. Silence lied. The next lie is a box that stops too early.
