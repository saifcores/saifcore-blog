...
excerpt: >-
  Jeudi 16:47. Un payout marchand. Le client timeout, puis renvoie. Puis encore.
  Quatre-vingt-dix secondes d'un triple paiement.
kind: writing
...
C'est la version qui a été livrée. Celle qui a failli l'être a payé le même marchand **trois fois** pour une seule vente.
...
Un client partenaire soumet un payout : **85 000 XOF**, un marchand, une commande. Le provider mobile money ne tombe pas. Il devient **lent** — quatre secondes, puis cinq, puis huit. Le temps habituel se compte en quelques centaines de millisecondes. Le timeout du client est à trois secondes.

À 16:47:03, le client décide que la première requête a échoué.

Elle n'avait pas échoué. Elle était encore dans le tuyau.
...
En environ quatre-vingt-dix secondes, la même commande de payout a été soumise **trois fois**. Côté fournisseur, les trois auraient abouti — trois virements distincts, même montant, même bénéficiaire, une seule vente.

La finance ne l'aurait pas vu cet après-midi-là. Elle l'aurait vu dans un fichier de fin de journée — ou des jours plus tard — sans piste évidente vers un timeout de trois secondes.
...
Si vous recrutez pour les paiements, c'est le jugement que vous voulez dans la pièce : les retries sont inévitables ; le silence n'autorise pas à bouger l'argent une seconde fois.
...
Les trois retries portaient la même clé. Le serveur a reconnu les appels deux et trois comme des doublons d'une commande déjà en cours, renvoyé la réponse d'origine, et n'a jamais émis de second virement.
...
| **Support**     | Un marchand qui demande pourquoi il a été payé deux fois | Aucun ticket                             |
| **Engineering** | Un incident review                                       | Une ligne de log, lue le lendemain matin |
...
Si la clé avait été un UUID frais à chaque retry, le payout du jeudi aurait atterri trois fois. Le fichier de règlement du vendredi aurait été « trop juste » — des crédits en trop, pas des lignes manquantes. Le produit aurait appelé ça une victoire. La finance, une fuite. Quelqu'un aurait été au téléphone avec le marchand, pour expliquer un remboursement sur de l'argent déjà dépensé.
...
Même règle que dans [systèmes fintech à l'échelle](/fr/articles/scalable-fintech-systems) : dix retries doivent égaler **une** intention économique. Si la clé peut changer entre deux tentatives, le filet de sécurité n'a jamais vraiment existé.
...
Un mot a menti. Le silence a menti. Le prochain mensonge, c'est un schéma qui s'arrête trop tôt.
